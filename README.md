...
We present the data which was generated by the above evaluated tools i.e., R-CPATMiner and RefactoringMiner. The dataset is generated by running the tools over 1000 open source machine learning projects. The project list is specified in this [file](https://drive.google.com/file/d/1Szd9qf0SqNwFXP7uYeh06yaQDwHKnsQs/view?usp=sharing). Reviewers can regenerate all the data using the above tools. However, we do not expect the reviewers to execute the tools on this dataset because it takes a long time to complete and cannot be completed in less than 30 minutes.
